<script setup lang="ts">
import { type DropdownItemProps } from "hy-element";

const items: DropdownItemProps[] = [
  { command: "1", label: "Action 1" },
  { command: "2", label: "Action 2" },
  { command: "3", label: "Action 3", disabled: true },
  { command: "4", label: "Action 4", divided: true },
];
</script>

<template>
  <div class="row">
    <div class="col">
      <div class="desc">通过 slot 传入 item</div>
      <hy-dropdown>
        <span class="dropdown-link">
          Dropdown List
          <hy-icon icon="angle-down" />
        </span>
        <template #dropdown>
          <hy-dropdown-item command="1">Action 1</hy-dropdown-item>
          <hy-dropdown-item command="2" label="Action 2" />
          <hy-dropdown-item command="3" disabled>Action 3</hy-dropdown-item>
          <hy-dropdown-item command="4" divided>Action 4</hy-dropdown-item>
        </template>
      </hy-dropdown>
    </div>
    <div class="col">
      <div class="desc">通过 props 传入 item</div>
      <hy-dropdown :items="items">
        <span class="dropdown-link">
          Dropdown List
          <hy-icon icon="angle-down" />
        </span>
      </hy-dropdown>
    </div>
  </div>
</template>

<style scoped>
.row {
  display: flex;
  flex-wrap: wrap;
}
.col {
  flex: 1;
}
.desc {
  display: block;
  color: var(--hy-text-color-secondary);
  font-size: 14px;
  margin-bottom: 20px;
}
.dropdown-link {
  display: flex;
  align-items: centhy;
  cursor: pointhy;
  color: var(--hy-color-primary);
  i {
    margin-left: 8px;
  }
}
</style>
