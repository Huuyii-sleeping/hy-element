...
import { getLastBottomOffset } from './methods.ts'
...
<template>
    <Transition :name="transitionName" @enter="boxHeight = messageRef!.getBoundingClientRect().height"
        ...
        <div ref="messageRef" class="hy-message" :class="{
            [`hy-message--${type}`]: type,
            'is-close': showClose,
            'text-center': center,
        }" :style="cssStyle" v-show="visible" role="alert" @mouseenter="clearTimer" @mouseleave="startTimmer">
            <hy-icon class="hy-message__icon" :icon="iconName" />
            <div class="hy-message__content">
                <slot>
                    <render-vnode v-if="message" :vNode="message" />
                </slot>
            </div>
            <div class="hy-message__close" v-if="showClose">
                <hy-icon icon="xmark" @click.stop="close" />
            </div>
        </div>
    </Transition>
</template>
